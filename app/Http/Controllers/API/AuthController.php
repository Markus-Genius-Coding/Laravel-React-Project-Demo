...
use App\Jobs\NotifyAuthTokenRevokedJob;
use App\Models\User;
...
class AuthController extends Controller
{
    ...
    public function doLogin(UserLoginRequest $request)
    {
        ...
        if ($user == null || !Auth::attempt($request->toArray())) {
            ...
            $result = [
                ...
                'authdata' => $this->getTokenAndRefreshToken(strtolower($request->email), $request->password)->original,
                'userdata' => $user,
            ];
    ...
    private function revokeAccessAndRefreshTokens($token) {

        NotifyAuthTokenRevokedJob::queueJob($token->id);
